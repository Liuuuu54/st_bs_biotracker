...
test('bsAddSperm 仅插入、体外射精、保护措施或事实不明时不登记精液', () => {
  for (const facts of [{ ejaculatedInside: false, protected: false }, { ejaculatedInside: true, protected: true }, {}]) {
    const cs = makeChatState()
    const result = applyToolCall(cs, {
      name: 'bsAddSperm',
      arguments: { female: 'F', male: 'A', race: '人类', amount: 20, ...facts },
    })
    assert.equal(result.applied, false)
    assert.deepEqual(cs.characters.F.profile.base.sperms || [], [])
    assert.deepEqual(cs.characters.F.profile.base.conceptionCandidates || [], [])
    assert.equal(cs.characters.F.profile.base.latestSexDays, undefined)
    assert.deepEqual(cs.characters.F.profile.experience || {}, {})
  }
})
test('bsAddSperm 明确无保护且射入体内时才登记精液', () => {
  const cs = makeChatState()
  const result = applyToolCall(cs, {
    name: 'bsAddSperm',
    arguments: { female: 'F', male: 'A', race: '人类', amount: 20, ejaculatedInside: true, protected: false },
  })
  assert.equal(result.applied, true)
  assert.equal(cs.characters.F.profile.base.sperms[0].value, 20)
  assert.equal(cs.characters.F.profile.base.conceptionCandidates[0].male, 'A')
})
