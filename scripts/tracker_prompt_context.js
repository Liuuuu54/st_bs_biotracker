...
export const TRACKER_VARIABLE_GUIDE_PROMPT = [
  ...
  '- penetrationState / penetrationSource 是当前可受孕生殖道的插入状态与来源；只按剧情事件用 bsAddSperm 更新，插入不等于射精。',
  '- age: 角色年龄，单位为年。',
  ...
  '- bsAssistFetalPosition 只在剧情明确出现人工、器械或魔法的胎位操作时调用（转位 rotate、托高 lift、推送 descend）。产兆前驱托高领头胎儿会延后分娩、推送会缩短前驱；托高要跟宫缩对抗，活力不足会被拒绝。每次操作都会带来一阵剧痛（反映在 laborPain），孕期则提高心理压力。',
