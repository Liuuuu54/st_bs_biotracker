...
export const TRACKER_VARIABLE_GUIDE_PROMPT = [
  ...
  '- 调用 bsAddSperm 时必须传 ejaculatedInside: true 和 protected: false；若射精位置或保护措施不明确，保持不调用，不得猜测。',
  '- conceptionCandidates: 本周期已经获得排卵受精竞争资格的父源快照；与 sperms 完全独立，清空 sperms 不会删除这里的记录。',
  '- conceptionCandidates[*] 保存 male、race、derivedType 和 competitionWeight；新的 bsAddSperm 会自动登记；手动切入排卵期时，当前仍有残留的 sperms 也会补登记；排卵统一结算后清空。',
  '- eggs: 当前可受精卵子数。',
  '- libido: 性欲。',
