...
export const TRACKER_VARIABLE_GUIDE_PROMPT = [
  ...
  '- penetrationState: 可受孕生殖道的当前插入状态。idle=未插入；inserted=已插入且本轮尚可沉积精液；spent=沉积过精液但仍未拔出，再次沉积前必须重新 insert。inserted／spent 均可供 UI 显示插入状态。',
  '- penetrationSource: 当前插入来源；idle 时为 null。它只描述插入，不代表已经射精。',
  '- age: 角色年龄，单位为年。',
  '- race: 当前保存的种族字符串，可能带子类或混血，不再带 [derived] 前缀。',
  '- derivedType: 衍生类型字符串，如 不死-僵尸；没有则为 null。',
  '- sperms: 体内残留精液来源列表。残留每天自动衰减 10、归零即消失，通常 1-3 天内自然清空；当下总有效量越高，本次受孕越容易，胜出来源的有效量也会提高高产物种的卵群规模。受精成功本身不会扣除或清空残留，不需要每轮重复描写流出。',
  '- bsAddSperm 必须按 insert → deposit →（再次 insert 或 withdraw）的状态机调用。不同来源再次 insert 视为旧来源拔出、新来源立即接手的原子交棒，只保留当前 penetrationSource，不建立多人列表；旧来源不可再替新来源 deposit 或 withdraw。口交、肛交、体外射精、保险套内射精、手淫与体表接触不得调用；插入本身不会增加 sperms。',
  ...
  '- bsImplantEmbryo.count 是胎儿卡／有效后代候选数，不是植入卵枚数。若故事是植入一群十枚卵、其中只有一名能成为有效后代，必须传 count=1；系统会把十枚这类卵群规模记录在该胎儿卡的 clutchSize。',
  ...
  '- fetuses[*].clutchSize: 这一张胎儿卡所代表的整群卵／幼体数量，不是胎儿卡数量。剧情可描写产下整群卵，但其中只有该卡对应的一名有效后代会写入族谱；绝不能按 clutchSize 自行增加 fetusesCount、胎儿卡或孩子。',
  ...
  '- children[*].birthClutchSize: 出生时所属卵群的总卵／幼体数，只是出生背景；这笔 children 记录仍只代表其中唯一成功长大繁衍的有效后代。',
  ...
  '- 新衣可直接用 bsChangeOutfit 的 main/accessories 建立并穿上：scope=owned 收入长期衣柜，scope=temporary 仅保留到该衣物脱下。bsAddWardrobeItem 只收藏或更新长期衣物；bsRemoveWardrobeItem 永久删除。衣物可用整数 id 或准确名称引用。',
