...
export function buildRegistrySystemPrompt(settings, options = {}) {
  ...
  const prompt = [
    ...
    '- base.penetrationState: 当前可受孕生殖道插入状态，只能是 idle、inserted、spent。一般开场填 idle；明确正在插入且尚未沉积精液填 inserted；已经沉积精液但尚未拔出填 spent。',
    '- base.penetrationSource: inserted／spent 时的插入来源名字；idle 时必须为 null。插入状态与 sperms 分开，插入不等于射精。',
